...
class bootstrap_nav_menu_456shop_walker extends Walker_Nav_Menu  {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				...
				$class_names = $class_names ? ' class="noLava ' . esc_attr( $class_names ) . '"' : '';	
			}
			...
			if($depth==1){
				...
				if(in_array("divider", $classes)){
					...
					if ( 'product' == get_post_type($item->object_id) ){
						...
						$image = wp_get_attachment_image_src( get_post_thumbnail_id( $item->object_id ), 'shop' );
						...
						$item_output .= '<div '.$margin.' class="item portfolio-item-menu"><a'.$attributes.'>';
...
class bootstrap_list_pages_walker extends Walker_Page{
    ...
    function start_el(&$output,$page,$depth,$args,$current_page){
		...
		if($depth==0){
			...
			$output .= $indent . '<li class="noLava ' . $dropdown_submenu . ' ' . $css_class . '"><a ' . $data . ' href="' . get_permalink($page->ID) . '">' . $link_before . apply_filters('the_title',$page->post_title,$page->ID ) . $link_after;	
		}

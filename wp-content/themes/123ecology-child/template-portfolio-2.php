			...
			<div class="container">
				<div class="row post-page">
					<div class="span12">
						...
						<div class="post_image shop-image">	
						<?php $post_thumbnail_id = get_post_thumbnail_id(); ?> 
						<?php $alt = get_post_meta($post_thumbnail_id, '_wp_attachment_image_alt', true);?>
						<img alt="<?php echo $alt; ?>" src="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'blog-style-2' ); echo $image[0];?>" />
						</div>
						<?php }?>
						<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
							<?php the_content();?>
							...
							<div class="portfolio-item">
								<?php if ( $lightbox ) { ?>
									<?php if(has_post_thumbnail()) {?>
		                                <a rel="prettyPhoto[pp_gal-<?php echo $post->ID ?>]" class="effect-thumb" href="<?php if ($videoURL) { echo $videoURL_raw; }else{ $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'large' ); echo $image[0]; }?>" title="<?php if ($videoURL) { the_title(); }else{ $attachment = get_post(get_post_thumbnail_id( $post->ID )); echo $attachment->post_title;}?>">
								...
								<div class="content">
									...
									<div class="column">
										<div class="post_content">
											...
											<?php if ( $linkURL ) { ?>
											<a class="more-link" href="<?php echo $linkURL; ?>">[<?php _e( 'read more', GETTEXT_DOMAIN_CHILD);?>]</a></p>
											<?php }else{?>
											<a class="more-link" href="<?php the_permalink(); ?>">[<?php _e( 'read more', GETTEXT_DOMAIN_CHILD);?>]</a></p>
											<?php }?>
										</div>
									</div>

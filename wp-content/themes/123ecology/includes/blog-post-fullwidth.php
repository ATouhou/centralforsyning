		...
		<?php $share = get_post_meta($post->ID, 'share_value', true);?>
						...
						<div class="<?php $allClasses = get_post_class(); foreach ($allClasses as $class) { echo $class . " "; } ?>">
							<?php if ( (function_exists('has_post_thumbnail')) && (has_post_thumbnail()) ) {?>
								<div class="blog-thumbnail bw-effect effect-thumb <?php if (!$share){?>effect-thumb-2<?php }?>">
									<img class="bw_Thumbnail" alt="<?php the_title(); ?>" src="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'blog-style-2-gray' ); echo $image[0];?>"/>
									<img class="Thumbnail" alt="<?php the_title(); ?>" src="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'blog-style-2' ); echo $image[0];?>"/>
		                    ...
		                    <?php } elseif ( $videoURL ) {?>
		                    	<iframe class="scale-with-grid" width="1170" height="658" src="<?php echo $videoURL ?>?wmode=transparent;showinfo=0" frameborder="0" allowfullscreen></iframe>
		                    <?php }?>
							<div class="content">
								<div class="post_type">
									<div class="icon 
